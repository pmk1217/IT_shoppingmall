...
public class UserProductSortByViewsDAO extends MySQLConnector {

	Connection conn = null;

	public UserProductSortByViewsDAO() {
		this.conn = getConnection();
	}

	public List<UserProductVO> productSortByViews(String category) {

		List<UserProductVO> ProductList = new ArrayList<UserProductVO>();

		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = "SELECT * FROM product where category = ? ORDER BY views DESC";
		...
		try {
			...
			System.err.println("productSortByViews() ERR : " + e.getMessage());
		} finally {
			close(conn, pstmt, rs);
		}
