...
public class UserProductSortByPriceDAO extends MySQLConnector {

	Connection conn = null;

	public UserProductSortByPriceDAO() {
		this.conn = getConnection();
	}

	public List<UserProductVO> productSortByPrice(String category) {

		List<UserProductVO> ProductList = new ArrayList<UserProductVO>();

		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = "SELECT * FROM product where category = ? ORDER BY price"; // 오름차순으로 가격순 정렬
//		String query = "SELECT * FROM product ORDER BY price DESC"; // 내림차순으로 가격순 정렬
		...
		try {
			...
			System.err.println("productSortByPrice() ERR : " + e.getMessage());
		} finally {
			close(conn, pstmt, rs);
		}
