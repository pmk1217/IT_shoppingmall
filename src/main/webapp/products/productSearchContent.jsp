...
<%
List<UserProductVO> ContentSearchList = (List<UserProductVO>) request.getAttribute("ContentSearchList");
System.out.println("products.jsp 실행 중 : " + ContentSearchList);
%>
...
<link rel="stylesheet"
   href="${contextPath}/assets/css/fontawesome.min.css">
...
.ContentSearch-item {
   width: calc(30% - 20px); /* 네모박스 간격 포함하여 너비 설정 */
   height: 400px;
   margin-bottom: 20px;
   padding: 10px;
         ...
         <div class="col-lg-9">
               ...
               <div class="col-md-6">
                  <ul class="list-inline shop-top-menu pb-3 pt-1">
                     <li class="list-inline-item"><a
                        ...
                        href="${contextPath}/userfilter/product/sortByLikes?category=${sessionScope.category}">
                           인기순 </a></li>
                     <li class="list-inline-item"><a
                        class="h3 text-dark text-decoration-none mr-3"
                        href="${contextPath}/userfilter/product/sortByCreaetedAt?category=${sessionScope.category}">최신순</a>
                     </li>
                     <li class="list-inline-item"><a
                        class="h3 text-dark text-decoration-none"
                        href="${contextPath}/userfilter/product/sortByViews?category=${sessionScope.category}">조회순</a>
                     </li>
                     <li class="list-inline-item"><a
                        class="h3 text-dark text-decoration-none"
                        href="${contextPath}/userfilter/product/sortByPrice?category=${sessionScope.category}">가격순</a>
                     </li>
                  </ul>
               </div>
                     ...
                     <div style="float: right; margin-left: 5px;">
                        <form action="${contextPath}/userfilter/product/search"method="post">
                           ...
                           <input type="text" name="SearchText" placeholder="검색어를 입력하세요"
                              ...
                              style="height: 27px; background-color: white; border: 1px solid black;">
            ...
            <!-- 검색 결과 -->
               <table id="pdtable" style="display: flex; flex-wrap: wrap; margin: 0 auto">
                  <c:choose>
                     <c:when test="${empty ContentSearchList}">
                        <!-- if() 부분 -->
                        <tr>
                     ...
                     <c:otherwise>
                        <c:forEach var="ContentSearch" items="${ContentSearchList}"
                           varStatus="status">
                           <tbody
                              style="margin-left: 18px; margin-bottom: 15px; border: 1px solid #ddd; width: 220px; height: 330px;">
                              <tr>
                                 <td align="center" colspan="3"><a
                                    href="${contextPath}/userfilter/product/detail?product_id=${ContentSearch.getProduct_id()}">
                                       <img
                                       src="${pageContext.request.contextPath}/images/${ContentSearch.getCategory()}/${ContentSearch.getImg()}"
                                       alt="${ContentSearch.getName()}"
                                       onclick="handleClick('${contextPath}/userfilter/product/detail?product_id=${ContentSearch.getProduct_id()}', '${ContentSearch.getProduct_id()}' )">
                                 </a></td>
                              </tr>

                              <tr>
                                 <td align="center" colspan="3" height="80px;"><c:out
                                       value="${ContentSearch.getName()}" /></td>
                              </tr>

                              <tr>
                                 <td align="left"
                                    style="color: black; font-size: 10px; font-weight: bold">조회
                                    수 : ${ContentSearch.getViews()}</td>
                              </tr>

                              <tr>
                                 <td><input class="fas fa-heart"
                                    style="color: red; margin-left: 2px;" type="button"
                                    id="likes${ContentSearch.getProduct_id()}" value="좋아요"
                                    onclick="handleLike('${ContentSearch.getProduct_id()}')" />
                                    <b id="likes${ContentSearch.getProduct_id()}"
                                    style="font-size: 14px; margin-left: 1px;">${ContentSearch.getLikes()}
                                 </b></td>
                                 <td align="right"
                                    style="color: red; font-size: 18px; font-weight: bold"><fmt:formatNumber pattern="#,##0" value="${ContentSearch.getPrice()}" />원</td>
                                 <!--c:out  -->
                              </tr>
                           </tbody>
                        </c:forEach>
